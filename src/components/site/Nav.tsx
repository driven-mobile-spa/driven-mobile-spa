        ...
        <Link to="/" className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-base font-extrabold uppercase italic tracking-tight sm:text-lg">
            ...
            <span className="text-brand-blue">
              {site.businessName.split(" ").slice(1).join(" ")}
            </span>
          </span>
        </Link>
          ...
          {links.map((l) =>
            l.hash ? (
              <Link
                key={l.label}
                to={l.to}
                className={`transition-colors hover:text-white ${isHome ? "text-white" : ""}`}
              >
                {l.label}
              </Link>
            ) : (
              <Link
                key={l.label}
                to={l.to}
                className={`transition-colors hover:text-white ${location.pathname === l.to ? "text-white" : ""}`}
              >
                {l.label}
              </Link>
            ),
